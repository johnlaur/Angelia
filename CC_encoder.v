...
Format changed to match Protocol Discussion paper 5 Dec 2014
 ...
 This is priority C&C data from the hardware. It is sent whenever data changes and every **mS	
		 ...
		 RAM Byte Data
				...
				2	Seq #							[15:8]
				3	Seq #							[7:0]
			0	4	Bits - PTT, Dot, Dash	[0] = PTT, [1] = Dot, [2] = Dash, [3] = new_frequency
...
begin 
	...
	{memory[0],    temp[0]}  <=  {temp[0],  4'b0, new_frequency, Dash, Dot, PTT};  // sent in real time
	...
	// RAM 47 - 54 not presently used - intialised to zero
...
always @ (posedge clock)	
begin
	if (counter == (update_rate * 125000)) begin	
		counter <= 25'd0;
	...
	0: begin 
			...
			if ((memory[0] != previous[0]) || (memory[55] != previous[1])) begin 
				...
				for (count = 0 ; count  < 6'd57 ; count = count + 6'd1)  // update all status 
